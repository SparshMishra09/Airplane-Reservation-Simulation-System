...
export class BookingsService {
  ...
  /**
   * Partial seat change: only swap the specified old→new pairs.
   * Seats NOT listed in seatChanges remain untouched.
   */
  async changeSeatPartial(
    id: string,
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      for (const { oldSeat, newSeat } of seatChanges) {
        // Find the old seat in this booking
        const oldSeatRecord = booking.seats.find((s) => s.seatNumber === oldSeat);
        if (!oldSeatRecord) {
          throw new NotFoundException(`Seat ${oldSeat} is not part of booking ${id}`);
        }
        ...
        if (existingNew) {
          if (!existingNew.isAvailable) {
            // Roll back: re-reserve old seat before throwing
            await tx.seat.update({
              where: { id: oldSeatRecord.id },
              data: { isAvailable: false, bookingId: booking.id },
            });
          ...
          try {
            ...
            if (e.code === 'P2002') throw new ConflictException(`Seat ${newSeat} is already booked!`);
            throw e;
          }
        }
      }

      // Recalculate total from freshly-updated seats
      ...
      const baggageCost = (booking.checkedBags || 0) * 50;
      const mealCost =
        booking.meal === 'Vegetarian' ? 15 : booking.meal === 'Vegan' ? 20 :
        booking.meal === 'Halal' ? 25 : booking.meal === 'Kosher' ? 30 : 0;
      const insuranceCost = booking.hasCancellationProtection ? 35 : 0;
